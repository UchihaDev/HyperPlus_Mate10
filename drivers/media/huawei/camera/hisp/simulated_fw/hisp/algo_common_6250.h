...
#define SENSOR_MAX_HDC_OTP_SIZE      600
...
#define MAX_OTP_SIZE            (0x4362)  /*expand 0x37e2-0x3878 for sfr data*//*expand to 0x4362 for Bac data*/
...
typedef struct
{
    union {
        struct {
            ...
            unsigned char otp_lsc_cal_flag;   /* 0x00 means sensor use LSC OTP data, 0xFF means platform use LSC OTP data,0xF0 means platform use altek lsc calibration algo*/
            char hdc[SENSOR_MAX_HDC_OTP_SIZE];
            char stationInfo[MAX_STATION_INFO_OTP_SIZE];
            char iso1[2];            /* ISO(High, Low)  for Sub Camera*/
            char awb1[6];            /* Gain_R(High,Low), Gain_G(High, Low), Gain_B(High,Low), for Sub Camera*/
            char lsc1[LSC_OTP_SIZE]; /* LSC Gain For Grid  for Sub Camera*/
            char dual_af[48];        /* auto focus info for dual camera */
            char dual_HDC[9919];     /* HDC info for dual camera */
            char dual_BPT[8002];     /* Bad pixel info for dual camera */
            char CCT[18];            /* CCT info for dual camera */
        };
