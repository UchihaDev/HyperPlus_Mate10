...
#define MAX_STRIPE_NUM         10 // double check me
//add for cc
...
// add for gamma
#define TONE_CURVE_LEN          257
#define GAMMA_CALIBRATION_SIZE  257
#define GAMMA_CURVE_HW_SIZE     33 // DIFFER, isp 33
#define GAMMA_FE_NUM            33
...
//for common_info
#define LSC_OTP_SIZE            (41 * 31 * 4 * 14 / 8)
#define PDAF_OTP_SIZE           (PDAF_CORRECT_MAP + PDAF_DEFOCUS_COEF + PDAF_HW_SLOPE_DATA)
#define MAX_OTP_SIZE            (0x6E2D)/*ox8000 in chicago*/
...
//for fd
#define FD_MAX_NUM              20

#define PIPE_MAX               2
#define METASIZE_NOT_EQUAL_FLAG 970
...
typedef struct
{
    ...
    unsigned char reserved[3]; //DIFFER, DONE
} sensor_otp_t;

/* 3A stat structure */
typedef struct _awb_stat_t
{
    //unsigned int y_sum_stat[AWB_GRAY_BLK_ROWS* AWB_GRAY_BLK_COLS];
    // unsigned int rg_sum_stat[AWB_GRAY_BLK_ROWS* AWB_GRAY_BLK_COLS];
    // unsigned int bg_sum_stat[AWB_GRAY_BLK_ROWS* AWB_GRAY_BLK_COLS];
    // unsigned int ctrst_sum_stat[AWB_GRAY_BLK_ROWS* AWB_GRAY_BLK_COLS];//only in v150
    // unsigned int count_sum_stat[AWB_GRAY_BLK_ROWS* AWB_GRAY_BLK_COLS];
    ...
    HI_S64       green_gravity_rg;//371s21.10bit£¬Ö»¶Á
    HI_S64       green_gravity_bg;//371s21.10bit£¬Ö»¶Á
    //unsigned int green_gravity_rg;
    //unsigned int green_gravity_bg;
    unsigned int green_count;
    // unsigned int r_hist[AWB_SINGLE_CH_HIST_SIZE];
    // unsigned int g_hist[AWB_SINGLE_CH_HIST_SIZE];
    // unsigned int b_hist[AWB_SINGLE_CH_HIST_SIZE];
    unsigned int r_hist[AWB_HISTOGRAM_NUM];
    unsigned int g_hist[AWB_HISTOGRAM_NUM];
    unsigned int b_hist[AWB_HISTOGRAM_NUM];

    HI_U32 y_sum0[AWB_STAT_EXT_BLKS]; //28bit 26
    HI_U32 rg_sum0[AWB_STAT_EXT_BLKS]; //30bit, 20.10
    HI_U32 bg_sum0[AWB_STAT_EXT_BLKS]; //30bit, 20.10
    HI_U32 ctrst0[AWB_STAT_EXT_BLKS]; //28
    HI_U32 count0[AWB_STAT_EXT_BLKS]; //16

    HI_U32 y_sum1[AWB_STAT_EXT_BLKS]; //28bit 26
    HI_U32 rg_sum1[AWB_STAT_EXT_BLKS]; //30bit, 20.10
    HI_U32 bg_sum1[AWB_STAT_EXT_BLKS]; //30bit, 20.10
    HI_U32 ctrst1[AWB_STAT_EXT_BLKS]; //28
    HI_U32 count1[AWB_STAT_EXT_BLKS]; //16

    HI_U32 y_sum2[AWB_STAT_EXT_BLKS]; //28bit 26
    HI_U32 rg_sum2[AWB_STAT_EXT_BLKS]; //30bit, 20.10
    HI_U32 bg_sum2[AWB_STAT_EXT_BLKS]; //30bit, 20.10
    HI_U32 count2[AWB_STAT_EXT_BLKS]; //16

    HI_U32 awb_2d_hist[32 * 128]; //26bit

    //Delete it, it is only used by DRC firmware
    //HI_U32 y_avg_sum_stat[AWB_GRAY_BLK_ROWS* AWB_GRAY_BLK_COLS]; //only in v150
...
typedef struct _ae_stat_t
{
    ...
    //unsigned int      ae_luma_hist[AE_LUM_HISTOGRAM_SIZE];
    unsigned int      luma_hist[AE_LUM_HISTOGRAM_SIZE];
    unsigned int      ae_matrix_avg[32 * 32];
    //unsigned int      ae_hist_matrix[AE_4BIN_HISTOGRAM_SIZE];
    unsigned int      block_4bin_hist[AE_4BIN_HISTOGRAM_SIZE];
    //unsigned int      ae_roi_hist_matrix[AE_ROI_HISTOGRAM_SIZE];
    unsigned int      roi_hist_matrix[AE_ROI_HISTOGRAM_SIZE];
    unsigned int      ae_roi_luma_avg[AE_ROI_NUM];
    unsigned int      ae_roi_matrix_avg[AE_ROI_NUM];
    //unsigned int      roi_brightness[AE_ROI_NUM];
    unsigned int      ae_post_luma_hist[AE_LUM_HISTOGRAM_SIZE];
    unsigned int      ae_post_matrix_avg[32 * 32];
    unsigned int      ae_post_cb_matrix_avg[32 * 32];
    unsigned int      ae_post_cr_matrix_avg[32 * 32];
    unsigned int      ae_post_red_num[32 * 32];
    unsigned int      ae_post_blue_num[32 * 32];
    unsigned int      ae_post_color_num[AE_COLOR_NUM];
    unsigned int      lbc_luma_hist[AE_LUM_HISTOGRAM_SIZE];

} ae_stat_t;

#define AF_SPECIAL_WINDOW_NUM 4
#define AF_TRIGGER_BLOCK_NUM   16
#define AF_SET_NUM             3
typedef struct _af_stat_t
{

    //
    unsigned long long int  y_accu_val[13];  //yv
    unsigned int            brightness_hist[AF_SPECIAL_WINDOW_NUM *  AF_HIST_NUM]; //y_hist
    unsigned int            r_counter[AF_SPECIAL_WINDOW_NUM];
    unsigned int            g_counter[AF_SPECIAL_WINDOW_NUM];
    unsigned int            b_counter[AF_SPECIAL_WINDOW_NUM];
    unsigned int            gradient_hist[3 * 4 * 16];
    unsigned long long int  focus_val[3 * 13]; //fv
    unsigned int            trigger_yv[AF_TRIGGER_BLOCK_NUM];
    unsigned long long int  trigger_fv[AF_SET_NUM*AF_TRIGGER_BLOCK_NUM];
    unsigned int            counter[3 * 13];
    unsigned int reserved;
} af_stat_t;

typedef struct _v2_af_stat_t
{
    unsigned long long int  focus_val[13*3];//AF_WINDOW_NUM
    unsigned long long int  y_accu_val[13];
    unsigned int            counter[13*3];
