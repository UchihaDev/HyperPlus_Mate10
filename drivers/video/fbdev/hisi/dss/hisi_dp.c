...
int dp_pxl_ppll7_init(struct hisi_fb_data_type *hisifd, uint64_t pixel_clock)
{
	...
	ceil_temp = dp_ceil(vco_min_freq_output, pixel_clock);
	...
	vco_freq_output = ppll7_freq_divider * pixel_clock;
	...
	ret = clk_set_rate(hisifd->dss_pxl1_clk, DEFAULT_MIDIA_PPLL7_CLOCK_FREQ);

	if (ret < 0) {
		HISI_FB_ERR("fb%d dss_pxl1_clk clk_set_rate(%llu) failed, error=%d!\n",
			hisifd->index, pixel_clock, ret);
	}
...
static int dp_on(struct platform_device *pdev)
{
	...
	dptx->dptx_switching_source = false;
...
int dp_device_srs(struct hisi_fb_data_type *hisifd, bool ublank)
{
	...
	if (dptx->dptx_switching_source) {
		HISI_FB_ERR("dptx is switching source.\n");
		return -EINVAL;
	}
	...
	if (dptx->dptx_enable && dptx->video_transfer_enable) {
		if (ublank) {
			if (bpress_powerkey) {
				...
				dp_pxl_ppll7_init(hisifd, pinfo->pxl_clk_rate);
				bpress_powerkey = false;
			}
		} else {
			dptx_write_dpcd(dptx, DP_SET_POWER, DP_SET_POWER_D3);
			dptx_disable_default_video_stream(dptx);
			//clk_set_rate(hisifd->dss_pxl1_clk, DEFAULT_MIDIA_PPLL7_CLOCK_FREQ_SAVEMODE);
...
int hisi_dptx_switch_source(uint32_t user_mode, uint32_t user_format)
{
	...
	if (((pinfo->xres >= MAX_DIFF_SOURCE_X_SIZE) && (dptx->max_edid_timing_hactive > MAX_DIFF_SOURCE_X_SIZE))
		|| (dptx->user_mode != 0)) {
		/* DP plug out */
		dptx->dptx_switching_source = true;
