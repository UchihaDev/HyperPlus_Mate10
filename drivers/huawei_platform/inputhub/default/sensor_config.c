...
int send_calibrate_data_to_mcu(int tag, uint32_t subcmd, const void *data, int length, bool is_recovery)
{
	...
	if(pkg_mcu.errno!=0) {
		...
		__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__, "send calibrate data fail\n");
		return -1;
	} else {
...
int write_gsensor_offset_to_nv(char *temp, int length)
{
	...
	if (temp == NULL) {
		...
		__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__, "write_gsensor_offset_to_nv fail, invalid para!\n");
		return -1;
	}
...
int write_gyro_sensor_offset_to_nv(char *temp, int length)
{
	...
	if (temp == NULL) {
		...
		__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__, "write_gyro_sensor_offset_to_nv fail, invalid para!\n");
		return -1;
	}
...
int write_gyro_temperature_offset_to_nv(char *temp, int length)
{
	...
	if (temp == NULL) {
		...
		__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__, "write_gyro_temp_offset_to_nv fail, invalid para!\n");
		return -1;
	}
...
int write_magsensor_calibrate_data_to_nv(char *src)
{
	...
	if (NULL == src) {
		...
		__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__, "invalid para\n");
		return -1;
	}
...
int fill_extend_data_in_dts(struct device_node *dn, const char *name, unsigned char *dest, size_t max_size, int flag)
{
	...
	if (0 == len || len > max_size) {
		...
		__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__, "len err\n");
		return -1;
	}
	ret = of_property_read_u32_array(dn, name, buf, len);
	if (ret) {
		hwlog_err("In %s: read %s failed!\n", __func__, name);
		__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__, "read failed\n");
		return -1;
	}
