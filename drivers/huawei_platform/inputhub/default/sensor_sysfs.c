...
static int write_ps_offset_to_nv(int *temp)
{
	...
	if (temp == NULL) {
		...
		__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__, "write_ps_offset_to_nv fail, invalid para!\n");
		return -1;
	}
...
int write_als_offset_to_nv(char *temp)
{
	...
	if (temp == NULL) {
		...
		__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__, "write_als_offset_to_nv fail, invalid para!\n");
		return -1;
	}
...
static ssize_t store_airpress_set_calidata(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (ret != 0) {
		...
		__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__, "set airpress_sensor data failed\n");
		return -1;
	}
	if (pkg_mcu.errno != 0) {
		hwlog_err("set airpress_sensor sysfs offset fail,err=%d\n", pkg_mcu.errno);
		__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__, "set airpress_sensor offset fail\n");
	} else {
...
int ois_commu(int tag, unsigned int cmd, unsigned int pare, unsigned int responsed, bool is_subcmd)
{
	...
	if (responsed == NO_RESP){
		...
		if (ret) {
			...
			__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__, "send ois gyro cfg cmd failed\n");
			return ret;
		}
		if (pkg_mcu.errno != 0) {
			hwlog_err("set ois gyro cfg cmd fail,err=%d\n", pkg_mcu.errno);
			__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__, "set ois gyro cfg cmd fail\n");
		} else {
