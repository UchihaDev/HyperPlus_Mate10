...
static int hs_timing_config_kirin660[][TUNING_INIT_TIMING_MODE][TUNING_INIT_CONFIG_NUM] = {
        /* bus_clk,    div, drv_phase, sam_dly, sam_phase_max, sam_phase_min, input_clk */
	{ /*MMC*/
                {3200000, 7, 7, 0, 15, 15, 400000},             /* 0: LEGACY 400k */
                {480000000, 9, 6, 0, 1, 1, 48000000},   /* 1: MMC_HS */ /* ES 400M, 8div 50M */
...
static int hs_timing_config_kirin980[][TUNING_INIT_TIMING_MODE][TUNING_INIT_CONFIG_NUM] = {
        /* bus_clk,    div, drv_phase, sam_dly, sam_phase_max, sam_phase_min, input_clk */
        { /*MMC*/
                {3200000, 7, 7, 0, 15, 15, 400000},             /* 0: LEGACY 400k */
                {480000000, 9, 6, 0, 1, 1, 48000000},   /* 1: MMC_HS */ /* ES 400M, 8div 50M */
	...
	*enable_shift = 0;
	...
	if ((true == host->clk_change)) {
		...
		if (8 <= sam_phase_val && sam_phase_val <= 12)
			*use_sam_dly = 1;
	} else if (IS_CS_TIMING_CONFIG == host->is_cs_timing_config) {
		/*sd clk to ppll2*/
	...
	/*setting enable_shift range. adapt for kirin660,kirin980 and later chipsets*/
	...
	/*setting use_sample_dly range. adapt for kirin660,kirin980 and later chipsets*/
...
void dw_mci_shift_sdio_set(struct dw_mci *host,int sam_phase_val,int d_value,int * use_sam_dly,int *enable_shift)
{
	...
	*enable_shift = 0;
	...
	if ((true == host->clk_change)) {
		...
		dev_info(host->dev, "dw_mci_shift_sdio_set set sdio sam\n");
		/*sdio clk to ppll3*/
		if (8 <= sam_phase_val && sam_phase_val <= 12)
			*use_sam_dly = 1;
...
/*lint -restore*/
	if (of_device_is_compatible(of_node, "hisilicon,kirin660-dw-mshc")) {
		scperctrls         |= BIT_VOLT_OFFSET;
		bit_sdcard_o_sel18 |= BIT_VOLT_VALUE_18;
		sdio_rst |= BIT_RST_SDIO_MIAMI;
		memcpy(hs_timing_config, hs_timing_config_kirin660, sizeof(hs_timing_config));/* unsafe_function_ignore: memcpy */
	} else if (of_device_is_compatible(of_node, "hisilicon,kirin980-dw-mshc")) {
		scperctrls         |= BIT_VOLT_OFFSET;
		bit_sdcard_o_sel18 |= BIT_VOLT_VALUE_18;
		sdio_rst |= BIT_HRST_SDIO_ATLANTA;
		memcpy(hs_timing_config, hs_timing_config_kirin980, sizeof(hs_timing_config));/* unsafe_function_ignore: memcpy */
...
static int dw_mci_hs_set_controller(struct dw_mci *host, bool set)
{
	...
	if (set) {
		if (0 == id) {
			if (mmc1_sys_ctrl_base)
				writel(BIT_HRST_SDIO_ATLANTA, mmc1_sys_ctrl_base + MMC1_SYSCTRL_PERRSTEN0);
			else
		...
		if (0 == id) {
			if (mmc1_sys_ctrl_base)
				writel(BIT_HRST_SDIO_ATLANTA, mmc1_sys_ctrl_base + MMC1_SYSCTRL_PERRSTDIS0);
			else
...
static const struct of_device_id dw_mci_hs_match[] = {
	...
	{
	.compatible = "hisilicon,kirin660-dw-mshc",
	.data = &hs_drv_data,
	},
	{},
};
