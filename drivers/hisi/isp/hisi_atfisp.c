...
static u64 hisp_nesc_cpucfg_param_paddr(void)
{
    ...
    if(dev != NULL)
        return (u64)(dev->atfshrd_paddr + AP_A7_CPUCFG_OFFSET);
    else
        return 0;
}

static struct hisi_nsec_cpu_dump_s* hisp_nesc_cpucfg_param_vaddr(void)
{
    struct hisi_atfisp_s *dev = (struct hisi_atfisp_s *)&atfisp_dev;

    if(dev != NULL)
        if(dev->atfshrd_vaddr != NULL)
            return (struct hisi_nsec_cpu_dump_s*)(dev->atfshrd_vaddr + AP_A7_CPUCFG_OFFSET);
    return NULL;
}
