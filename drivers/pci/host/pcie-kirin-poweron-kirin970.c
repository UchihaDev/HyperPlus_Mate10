...
static void kirin_pcie_natural_cfg(struct kirin_pcie *pcie)
{
	...
	if (pcie->dtsinfo.chip_type == CHIP_TYPE_ES) {
		 /*pull up phy0_ana_pwr_en signal */
	       val = kirin_apb_phy_readl(pcie, SOC_PCIEPHY_CTRL33_ADDR);
	       val |= (0x1 << 1);
	       kirin_apb_phy_writel(pcie, val, SOC_PCIEPHY_CTRL33_ADDR);
	}
...
static int kirin_pcie_eco(struct kirin_pcie *pcie)
{
	...
	sramdata = pcie->phy_base + pcie->sram_phy_offset;
	...
	kirin_sram_phy_writel(pcie, 0x7CF, 0x2E1);
	kirin_sram_phy_writel(pcie, 0x7CF, 0x360);
...
int kirin_pcie_turn_on(struct pcie_port *pp, enum rc_power_status on_flag)
{
	...
	time = 100;
	if (pcie->dtsinfo.board_type != BOARD_FPGA) {
		val = kirin_apb_phy_readl(pcie, SOC_PCIEPHY_STATE0_ADDR);
