						...
						bool excl)
{
	...
	if (f2fs_encrypted_inode(dir) && f2fs_encrypted_fixed_inode(dir)) {
		struct dentry *_de_ = d_find_any_alias(dir);
		if (_de_)
			f2fs_msg(sbi->sb, KERN_NOTICE,
				"dir (%s) %lu child (%pd) fixed encrypt\n",
				_de_->d_name.name, dir->i_ino, dentry);
		else
			f2fs_msg(sbi->sb, KERN_NOTICE,
				"dir (null) %lu child (%pd) fixed encrypt\n",
				dir->i_ino, dentry);
		dput(_de_);
		return -EPERM;
	}
	...
	if (IS_ERR(inode)) {
		if (PTR_ERR(inode) == -EINVAL) {
			struct dentry *_de_ = d_find_any_alias(dir);
			if (_de_)
				f2fs_msg(sbi->sb, KERN_NOTICE,
					"dir (%s) %lu child (%pd) cannot get new inode\n",
					_de_->d_name.name, dir->i_ino, dentry);
			else
				f2fs_msg(sbi->sb, KERN_NOTICE,
					"dir (null) %lu child (%pd) cannot get new inode\n",
					dir->i_ino, dentry);
			dput(_de_);
		}
		...
		unsigned int flags)
{
	...
	if (f2fs_encrypted_inode(dir)) {
		...
		if (res && res != -ENOKEY) {
			...
			if (_de_)
				f2fs_msg(sbi->sb, KERN_NOTICE,
					"dir (%s) %lu lookup %pd get encrypt info fail %d\n",
					_de_->d_name.name, dir->i_ino, dentry, res);
			else
				f2fs_msg(sbi->sb, KERN_NOTICE,
					"dir (null) %lu lookup %pd get encrypt info fail %d\n",
					dir->i_ino, dentry, res);
			dput(_de_);
			return ERR_PTR(res);
		}
	}
	...
	if (!de) {
		if (IS_ERR(page)) {
			...
			if (_de_)
				f2fs_msg(sbi->sb, KERN_NOTICE,
					"dir (%s) %lu lookup %pd get dentry page fail %ld\n",
					_de_->d_name.name, dir->i_ino, dentry, PTR_ERR(page));
			else
				f2fs_msg(sbi->sb, KERN_NOTICE,
					"dir (null) %lu lookup %pd get dentry page fail %ld\n",
					dir->i_ino, dentry, PTR_ERR(page));
			dput(_de_);
	...
	if (IS_ERR(inode)) {
		struct dentry *_de_ = d_find_any_alias(dir);
		if (_de_)
			f2fs_msg(sbi->sb, KERN_NOTICE, "dir (%s) %lu get inode (%pd) fail %ld\n",
				_de_->d_name.name, dir->i_ino, dentry, PTR_ERR(inode));
		else
			f2fs_msg(sbi->sb, KERN_NOTICE, "dir (null) %lu get inode (%pd) fail %ld\n",
				dir->i_ino, dentry, PTR_ERR(inode));
		dput(_de_);
