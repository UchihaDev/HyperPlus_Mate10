	...
	if (del > 0) {
		if (f2fs_test_and_set_bit(offset, se->cur_valid_map)) {
			...
			if (f2fs_dclient && !dsm_client_ocuppy(f2fs_dclient)) {
				...
				dsm_client_record(f2fs_dclient,
					"%s: alloc blkaddr error: 0x%lx, segno: %d (all: %d),\
					offset: %d, new_vblocks: %d, type: %d, alloc_type: %s\n",
					__func__, (long unsigned int)blkaddr, segno,
		...
		if (!f2fs_test_and_clear_bit(offset, se->cur_valid_map)) {
			...
			if (f2fs_dclient &&!dsm_client_ocuppy(f2fs_dclient)) {
				struct curseg_info *curseg = CURSEG_I(sbi, se->type);
				dsm_client_record(f2fs_dclient,
					"%s: delete blkaddr error: 0x%lx, segno: %d (all: %d),\
					offset: %d, new_vblocks: %d, type: %d, alloc_type: %s\n",
					__func__, (long unsigned int)blkaddr, segno,
					MAIN_SEGS(sbi), offset, new_vblocks,
					se->type, curseg->alloc_type ? "SSR": "LFS");
