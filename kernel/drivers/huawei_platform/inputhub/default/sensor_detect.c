...
static uint8_t check_file_list(uint8_t file_count, uint16_t *file_list)
{
	...
	if ((file_count == 0) || (NULL == file_list)) {
		...
		__dmd_log_report(DSM_SHB_ERR_IOM7_DYNLOAD, __func__, "val invalid\n");
		return 0;
	}
...
static int get_adapt_sensor_list_id(void)
{
	...
	if (!sensorhub_node) {
		hwlog_err("%s, can't find node sensorhub\n", __func__);
		__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__, "can't find node sensorhub\n");
		return -1;
	}
	prop = of_find_property(sensorhub_node, name, NULL);
	if (!prop) {
		hwlog_err("%s! prop is NULL!\n", __func__);
		__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__, " prop is NULL\n");
		return -EINVAL;
	}
	if (!prop->value) {
		hwlog_err("%s! prop->value is NULL!\n", __func__);
		__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__, "prop->value is NULL\n");
		return -ENODATA;
	}
	len = prop->length / 4;
	if (of_property_read_u32_array(sensorhub_node, name, wia, len)) {
		hwlog_err("%s:read adapt_sensor_list_id from dts fail!\n", name);
		__dmd_log_report(DSM_SHB_ERR_IOM7_CFG_DATA, __func__, "read adapt_sensor_list_id from dts fail\n");
		return -1;
	}
	for (i = 0; i < len; i++) {
...
int send_fileid_to_mcu(void)
{
	...
	if (ret) {
		...
		__dmd_log_report(DSM_SHB_ERR_IOM7_DYNLOAD, __func__, "send file id to mcu fail\n");
		return -1;
	}
	if (pkg_mcu.errno != 0) {
		hwlog_err("file id set fail\n");
		__dmd_log_report(DSM_SHB_ERR_IOM7_DYNLOAD, __func__, "file id set fail\n");
		return -1;
	}
...
static int detect_i2c_device(struct device_node *dn, char *device_name)
{
	...
	if (of_property_read_u32(dn, "bus_number", &i2c_bus_num)
		...
		__dmd_log_report(DSM_SHB_ERR_IOM7_DYNLOAD, __func__, "read i2c bus_number or bus address or chip_id_register from dts fail\n");
		return -1;
	}
	...
	if (of_property_read_u32_array(dn, "chip_id_value", wia, len)) {
		...
		__dmd_log_report(DSM_SHB_ERR_IOM7_DYNLOAD, __func__, "read chip_id_value from dts fail\n");
		return -1;
	}
...
static void redetect_failed_sensors(void)
{
	int index = 0;
	char *sensor_ty = NULL;
	char *sensor_st = NULL;
	...
	check_detect_result(REDETECT_LATER);
}
...
static void sensor_detect_exception_process(uint8_t result)
{
	...
	if(result > 0)
	{
		...
		{
			hwlog_info("%s : redect sensor after detect all sensor %d \n",__func__,i);
			redetect_failed_sensors();
		}
	}
}
...
static int gyro_data_from_mcu(const pkt_header_t *head)
{
	...
	case SUB_CMD_SELFCALI_REQ:
		return write_gyro_sensor_offset_to_nv(((pkt_gyro_calibrate_data_req_t *)head)->calibrate_data, GYRO_CALIDATA_NV_SIZE);
