...
static void charge_core_safe_policy_handler(struct charge_core_data *data)
{
    ...
    data->vterm = data->vterm <= (vterm_max-asp.nondc_volt_dec) ? data->vterm : (vterm_max-asp.nondc_volt_dec);
...
static void charge_core_safe_policy_handler_segment(struct charge_core_data *data)
{
    ...
    if (asp.level < BASP_PARA_LEVEL && segment_flag < BASP_VOL_SEGMENT_COLUMN-1)
    {
        ...
        hwlog_info(BASP_TAG"asp.level:%d, segment_flag:%d, tmp_ratio:%d, data->ichg:%d, asp.learned_fcc*tmp_ratio/BASP_PARA_SCALE:%d, data->vterm:%d, vterm_max-asp.nondc_volt_dec:%d\n",
            asp.level, segment_flag, tmp_ratio, data->ichg, asp.learned_fcc*tmp_ratio/BASP_PARA_SCALE, data->vterm, vterm_max-asp.nondc_volt_dec);
        data->ichg = data->ichg <= asp.learned_fcc*tmp_ratio/BASP_PARA_SCALE ? data->ichg : asp.learned_fcc*tmp_ratio/BASP_PARA_SCALE;
        data->vterm = data->vterm <= (vterm_max-asp.nondc_volt_dec) ? data->vterm : (vterm_max-asp.nondc_volt_dec);
