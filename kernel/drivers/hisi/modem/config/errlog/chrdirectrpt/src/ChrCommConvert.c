...
#ifdef LLT_OS_LINUX
    memcpy(pstmodemHeader->aucProductName, aucProductName, 20);
#else
...
#ifdef LLT_OS_LINUX
            memcpy(p, pStruct, *pLen);    /* [false alarm]:已经通过ulSize来防护缓冲区溢出 */
#else
            memcpy_s(p, ulSize, pStruct, *pLen);	/* [false alarm]:已经通过ulSize来防护缓冲区溢出 */
#endif
