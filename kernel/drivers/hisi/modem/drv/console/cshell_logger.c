...
int cshell_log_open(struct log_usr_info *usr_info)
{
	...
	flags.cshell_open_flags=(((u32)(((u32)0x1<<CSHELL_PMOM))<<16)|((u32)0x1<<CSHELL_PMOM));
	ret = bsp_icc_send((u32)ICC_CPU_MODEM ,(u32)((ICC_CHN_IFC << 16) | IFC_RECV_FUNC_CSHELL),(u8*)&flags,(u32)sizeof(struct cshell_open_s));//打开
...
int cshell_log_close(struct log_usr_info *usr_info)
{
	...
	cshell_log_err("%s entry\n", __func__);
	flags.cshell_open_flags=(((u32)(((u32)0x1<<CSHELL_PMOM))<<16)&0xFFFF0000);
	ret = bsp_icc_send((u32)ICC_CPU_MODEM ,(u32)((ICC_CHN_IFC << 16) | IFC_RECV_FUNC_CSHELL),(u8*)&flags,(u32)sizeof(struct cshell_open_s));//打开
	if(ret!=(int)sizeof(struct cshell_open_s))
	{
...
{
	memcpy(dest, src, count); /*lint !e747*/
	return 0;
}
/*lint +e715*/
