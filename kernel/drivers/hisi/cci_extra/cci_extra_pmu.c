				     ...
				     const char *buf, size_t count)
{
	...
	/* cppcheck-suppress * */
	if (sscanf(buf, "%u %u %u %u %u", &val[0], &val[1], &val[2], &val[3], &val[4]) != NUM_LATENCY_CFG) /* unsafe_function_ignore: sscanf */
		return -EINVAL;
				     ...
				     const char *buf, size_t count)
{
	...
	/* cppcheck-suppress * */
	if (sscanf(buf, "%u %u %u %u %u", &val[0], &val[1], &val[2], &val[3], &val[4]) != NUM_OUTSTANDING_CFG) /* unsafe_function_ignore: sscanf */
		return -EINVAL;
				 ...
				 unsigned int offset1, unsigned int offset2)
{
	...
	/* cppcheck-suppress * */
	if (sscanf(buf, "0x%llx 0x%llx", &start, &end) != 2)/* unsafe_function_ignore: sscanf */
		return -EINVAL;
				     ...
				     unsigned int regs)
{
	...
	/* cppcheck-suppress * */
	n = sscanf(buf, "%u %u %u %u %u %u %u", &val[0], &val[1], &val[2], &val[3], &val[4], &val[5], &val[6]); /* unsafe_function_ignore: sscanf */
