...
int hisi_ambient_resume(struct platform_device *pdev)
{
	...
	if (hisi_ambient) {
		...
		pr_err("ambient resume avg_current = %d, LOW_CURRENT = %d \n", avg_current, LOW_CURRENT);
		if ((hisi_ambient->now.tv_sec - hisi_ambient->last_time.tv_sec) >= hisi_ambient->interval &&
			(avg_current <= LOW_CURRENT)) {
			/* time > interval (15 mins) and avg current is lower than LOW_CURRENT (50mA). */
			pr_err("%s time pass %ld > interval(%d s) , now_cc = %d, start_cc = %d, avg_current = %d < LOW_CURRENT.\n ",
			...
			for (i = 0; i < hisi_ambient->sensor_count ; i++) {
				...
				if (i == 0 || min_temp > ambient_temp/1000)
					min_temp = ambient_temp/1000;
			}
