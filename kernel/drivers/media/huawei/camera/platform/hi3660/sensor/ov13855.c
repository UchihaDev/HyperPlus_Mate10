        ...
        struct platform_device* pdev)
{
	...
	if (pdev->dev.of_node) {
		rc = hw_sensor_get_dt_data(pdev, &s_ov13855);
		if (rc < 0) {
			cam_err("%s failed line %d\n", __func__, __LINE__);
			goto ov13855_sensor_probe_fail;
		}
	} else {
		cam_err("%s ov13855 of_node is NULL.\n", __func__);
		goto ov13855_sensor_probe_fail;
	}

    s_ov13855.dev = &pdev->dev;

	rc = hwsensor_register(pdev, &s_ov13855.intf);
    if (0 != rc){
        cam_err("%s hwsensor_register fail.\n", __func__);
        goto ov13855_sensor_probe_fail;
    }
    rc = rpmsg_sensor_register(pdev, (void *)&s_ov13855);
    if (0 != rc){
