...
static void hisi_tuning_offline_clk_rate(struct hisi_fb_data_type *hisifd)
{
	...
	if (hisifd->need_tuning_clk && hisifd->dss_pri_clk) {
		clk_set_rate(hisifd->dss_pri_clk, hisifd->dss_clk_rate.dss_pri_clk_rate);
		HISI_FB_DEBUG("fb%d set dss_pri_clk_rate = %llu.\n",
			hisifd->index, hisifd->dss_clk_rate.dss_pri_clk_rate);
	}
}
...
static void hisi_recovery_online_clk_rate(struct hisi_fb_data_type *hisifd)
{
	...
	if (hisifd->need_tuning_clk && hisifd_list[PRIMARY_PANEL_IDX]) {
		clk_set_rate(hisifd->dss_pri_clk, hisifd_list[PRIMARY_PANEL_IDX]->dss_clk_rate.dss_pri_clk_rate);

		hisifd->dss_clk_rate.dss_pri_clk_rate = hisifd_list[PRIMARY_PANEL_IDX]->dss_clk_rate.dss_pri_clk_rate;
		hisifd->need_tuning_clk = false;

		HISI_FB_DEBUG("fb%d recovery set dss_pri_clk_rate = %llu.\n",
			hisifd->index, hisifd_list[PRIMARY_PANEL_IDX]->dss_clk_rate.dss_pri_clk_rate);
	}
}
