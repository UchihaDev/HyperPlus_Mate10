...
void ts_report_pen(struct ts_cmd_node* in_cmd, struct ts_cmd_node* out_cmd)
{
	...
	for (id = 0; id < TS_MAX_PEN_BUTTON; id++)
	{
		...
		else if(pens->buttons[id].status == TS_FINGER_PRESS){
			key_value = 1;
		} else{
			key_value = 0;
		}
	...
	if(pens->tool.tool_type == WACOM_RUBBER_TO_PEN){
			//hover
			input_report_abs(input, ABS_X, pens->tool.x);
			input_report_abs(input, ABS_Y, pens->tool.y);
	...
	input_report_abs(input, ABS_X, pens->tool.x);
	input_report_abs(input, ABS_Y, pens->tool.y);
	input_report_abs(input, ABS_PRESSURE, pens->tool.pressure);
	input_report_abs(input, ABS_TILT_X, pens->tool.tilt_x);
	input_report_abs(input, ABS_TILT_Y, pens->tool.tilt_y);
	input_report_key(input, BTN_TOUCH, pens->tool.tip_status);
	input_report_key(input, pens->tool.tool_type, pens->tool.pen_inrange_status);
	input_sync(input);
}
