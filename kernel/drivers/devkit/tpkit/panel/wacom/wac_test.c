...
static int wacom_actual_check(void)
{
	...
	for (y = 0; y < m_panelInfo.y; y++) {
		for (x = 0; x < m_panelInfo.x - 1; x++) {
			if ( (!( y < data->m_ini.calth_ignoreline[0] )) && ( y < m_panelInfo.y - data->m_ini.calth_ignoreline[2] ) &&
				(!( x < data->m_ini.calth_ignoreline[3] )) && ( x < m_panelInfo.x - data->m_ini.calth_ignoreline[1] )
...
int wacom_fac_test(struct ts_rawdata_info *info, struct ts_cmd_node *out_cmd)
{
	...
	if(ret != NO_ERR) {
		...
		strncat(info->result, "0F-1F-2F-3F-4F", strlen("0F-1F-2F-3F-4F"));
		strncpy(fail_reason, "request mem fail", sizeof(fail_reason));
		goto free_memory;
	}
	...
	if(ret != NO_ERR) {
		TS_LOG_ERR("%s, wacom_parse_threshold_file\n", __func__);
		strncat(info->result, "0F-1F-2F-3F-4F", strlen("0F-1F-2F-3F-4F"));
		strncpy(fail_reason, "parse file fail", sizeof(fail_reason));
		goto out;
	}
	...
	if(ret != NO_ERR){
		...
		TS_LOG_ERR("%s, enter_test_mode fail", __func__);
		goto out;
	...
	if(ret != NO_ERR) {
		...
		strncpy(fail_reason, "get rawdata fail", sizeof(fail_reason));
		goto out;
	} else {
	...
	//2F rawdata diff
	ret = wacom_rawdata_rx2rx_diff_check();
	if(NO_ERR != ret) {
		TS_LOG_ERR("%s, rawdata rx2rx diff check failed\n", __func__);
	}
	ret1 = wacom_rawdata_tx2tx_diff_check();
	if(NO_ERR != ret1) {
	...
	//3F  3:  noise delta  ---Act data ÅÐ¶¨ - Calibration Threshold
	...
	if(ret != NO_ERR) {
		...
		strncpy(fail_reason, "get actual data fail", sizeof(fail_reason));
		goto out;
	} else {
	...
	if(ret != NO_ERR) {
		...
		strncpy(fail_reason, "short test i2c fail", sizeof(fail_reason));
		goto out;
	} else {
		TS_LOG_INFO("%s, short test success\n", __func__);
	}

	ret = Result_Short_Check();
	if(ret != NO_ERR) {
	...
	if(ret != NO_ERR) {
		...
		strncpy(fail_reason, "exit test i2c fail", sizeof(fail_reason));
	}

out:
	if(ret != NO_ERR) {
		strncat(info->result, "-", strlen("-"));
		strncat(info->result, fail_reason, sizeof(fail_reason));
	}
	strncat(info->result, "-", strlen("-"));
	strncat(info->result, wac_data->chip_name, strlen(wac_data->chip_name));
	strncat(info->result, "-", strlen("-"));
	strncat(info->result, wac_data->tp_module_name, strlen(wac_data->tp_module_name));
	strncat(info->result, "-", strlen("-"));
	strncat(info->result, wac_data->lcd_module_name, strlen(wac_data->lcd_module_name));
	...
	for(case_num = 0;case_num < MAX_CASE_CNT; case_num++) {
		switch(case_num){
			...
			case ACTUAL_DATA_IGNORE_TOP:
				retval = wacom_get_single_item(buf, file_size, &buf_offset, &(data->m_ini.calth_ignoreline[0]), 1);
				if(retval != NO_ERR) {
					TS_LOG_ERR("%s, get ACTUAL_DATA_IGNORE_TOP failed\n", __func__);
					return retval;
				}
				TS_LOG_INFO("calth_ignoreline[0] = %d\n", data->m_ini.calth_ignoreline[0]);
				continue;
			case ACTUAL_DATA_IGNORE_BOTTOM:
				retval = wacom_get_single_item(buf, file_size, &buf_offset, &(data->m_ini.calth_ignoreline[1]), 1);
				if(retval != NO_ERR) {
					TS_LOG_ERR("%s, get ACTUAL_DATA_IGNORE_BOTTOM failed\n", __func__);
					return retval;
				}
				TS_LOG_INFO("calth_ignoreline[1] = %d\n", data->m_ini.calth_ignoreline[1]);
				continue;
			case ACTUAL_DATA_IGNORE_LEFT:
				retval = wacom_get_single_item(buf, file_size, &buf_offset, &(data->m_ini.calth_ignoreline[2]), 1);
				if(retval != NO_ERR) {
					TS_LOG_ERR("%s, get ACTUAL_DATA_IGNORE_LEFT failed\n", __func__);
					return retval;
				}
				TS_LOG_INFO("calth_ignoreline[2] = %d\n", data->m_ini.calth_ignoreline[2]);
				continue;
			case ACTUAL_DATA_IGNORE_RIGHT:
				retval = wacom_get_single_item(buf, file_size, &buf_offset, &(data->m_ini.calth_ignoreline[3]), 1);
				if(retval != NO_ERR) {
					TS_LOG_ERR("%s, get ACTUAL_DATA_IGNORE_RIGHT failed\n", __func__);
					return retval;
				}
				TS_LOG_INFO("calth_ignoreline[3] = %d\n", data->m_ini.calth_ignoreline[3]);
				continue;
